...
{
    /// <summary>
    /// 向Server取得玩者設定，傳入資料名稱List
    /// </summary>
    public static void GetPlayerSetting(List<string> _propertyNameList)//向Server取得玩者設定
    {
        try
        {
            ArcaletItem.GetItemInstance(ArcaletSetter.arcaletGame, ArcaletSetter.PlayerSettingIGuid, CallBack_GetPlayerSetting, _propertyNameList);
        }
        catch (Exception ex)
        {
            Debug.LogWarning(ex);
    ...
    * 需要資料格式的轉換 
    *******************************************************************/
    static void CallBack_GetPlayerSetting(int code, object data, object token)
    {
        //Code為0表示取得資料成功
        if (code == 0)
        {
            ...
            *******************************************************************/
            ...
            foreach (Hashtable hashAttribute in hashAttributeList)
            {
                ...
                {
                    Debug.LogWarning(string.Format("PlayerSetting資料表中的屬性名稱{0}不在欲取得的屬性名稱陣列中", propertyName));
                }
            }
        }
        //Code非0表示取得資料失敗
        ...
        {
            Debug.LogWarning("向Server取得PlayerSetting失敗 - Error:" + code);
            return;
        }
        ArcaletSetter.GetDataProgress("PlayerSetting");//標示為已向Server取得玩家設定
        Debug.Log("取得玩家設定資料成功!");
    }
}
