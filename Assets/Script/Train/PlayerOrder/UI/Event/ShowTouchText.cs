...
public class ShowTouchText : IShowText
{
    ...
    public void ShowText()
    {
        ...
        if (textAnimator.GetCurrentAnimatorStateInfo(0).IsName("stop"))
        {
            ...
            Dictionary<int, TrainScene.TouchFeedBack>.KeyCollection keyColl = TrainScene.OwnPTouchFeedBack[monsterStatus.Personality].Keys;
            //取得key值並抓取符合的親密度TouchFeedback ID
            foreach (int key in keyColl)
            {
                if (monsterStatus.Happy <= TrainScene.OwnPTouchFeedBack[monsterStatus.Personality][key].familiarMax
                        && monsterStatus.Happy >= TrainScene.OwnPTouchFeedBack[monsterStatus.Personality][key].familiarMin)
                {
                    feedBackTextIDList.Add(TrainScene.OwnPTouchFeedBack[monsterStatus.Personality][key].id);
                }
            }
            if (feedBackTextIDList.Count > 0)//feedBackTextIDList為0，代表沒有符合該個性快樂度區間的對話，則不執行
            {
                ...
                touchFeedBackText = TrainScene.OwnPTouchFeedBack[monsterStatus.Personality][feedBackTextIDList[rnd]].feedBackDialogue;
                //設定文字
                textTextMesh.text = WrapWord.GetWarpWord(touchFeedBackText, 20);
                //設定文字動畫
                textAnimator.SetTrigger(TrainScene.OwnPTouchFeedBack[monsterStatus.Personality][feedBackTextIDList[rnd]].textAni);
                //校正文字方向
                UpdateTextDirection();
            }
        }
